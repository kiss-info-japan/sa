...
レシピを詳細に紹介してください。
それが社会的にポピュラーな料理の場合、ユーザーに合わせてプラスワンポイントのユニークなアイデアも添えてください。
------------------------------------
《おすすめアイテム》
...
app.post('/print', async (req, res) => {
  const { text } = req.body;
  if (!text) return res.status(400).json({ error: "印刷テキストが必要です" });

  try {
    fs.writeFileSync(inputTxtPath, text, "utf8");

    exec(`python "${textToImagePy}"`, (err1, stdout1, stderr1) => {
      console.log("🖼 画像生成スクリプト実行:", stdout1);
      if (err1) {
        console.error("画像生成エラー:", err1);
        console.error("stderr:", stderr1);
        return res.status(500).json({ error: "画像生成に失敗しました" });
      }

      if (!fs.existsSync(outputPngPath)) {
        console.error("画像ファイルが存在しません: output.png が生成されていない");
        return res.status(500).json({ error: "画像ファイルが見つかりません" });
      }

      exec(`python "${printPy}" "${outputPngPath}"`, (err2, stdout2, stderr2) => {
        if (err2) {
          console.error("印刷エラー:", err2);
          return res.status(500).json({ error: "印刷に失敗しました" });
        }

        res.json({ message: "印刷が完了しました！" });
      });
    });
  } catch (error) {
